...
def inverted_nms(det, c, iou_thres):
    det[:,:4] = det[:,:4] + c
    ...
    for i in range(len(det)-1):
        ...
        det_accu[:, 0:4] = det_accu[:, 0:4]-det_accu[:, 6]
        det_accu[:, 0:4] = det_accu[:, 0:4] * np.tile(det_accu[:, -1:], (1, 4))
        det[i+1, 0:4] = np.sum(det_accu[:, 0:4], axis=0) / np.sum(det_accu[:, -1:]) + det[i+1, 6]
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        det = np.c_[x.cpu().numpy(), c.cpu().numpy()]
        i = inverted_nms(det, iou_thres)
        i = torch.tensor(i, device=boxes.device)
